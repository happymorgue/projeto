var idObjeto = window.location.pathname.split('/')[4];
...
function registoObjeto() {
    fetch('/api/convertUserEmailRegularId')
        .then(response => response.json())
        .then(data => {
...
function registarObjetoPerdido(idRegular) {
    ...
    sendImage().then(image => {
        ...
        fetch('/api/regular/dono/' + idRegular + '/atualizarObjetoPerdido/' + idObjeto, {
            method: 'POST',
            headers: {
